...
class DroneConnect(QObject):
    ...
    def __init__(self, websocket_ip, port=9090, name =''):
        ...
        self.ws.subscribe('/mavros/local_position/pose', PoseStamped(), name+"/local/pose", rate=5.0, queue_length=1)
        self.ws.subscribe('/geo/local_pose', PoseStamped(),name+"/geo/local_pose", rate=10.0, queue_length=1)
        self.ws.subscribe('/drone/diagnostics', Diagnostics(), name+"/diagnostics", rate=5.0, queue_length=1)
        self.ws.subscribe('/mavros/global_position/global', NavSatFix(), name+"/global/pose", rate=5.0, queue_length=1)
