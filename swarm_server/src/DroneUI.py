...
class Ui(QtWidgets.QWidget):
    ...
    def initSlots(self):
        ...
        self.drone_client.diag_signals.connect(self._getDiag)
        self.drone_client.ws.connect_signal.connect(self._changeConnect)

    def _getDiag(self):
        self._changeGuiElem(self.drone_client.diagnostics)
    ...
    def setMode(self, mode):
        self.drone_client.set_mode(mode)
    ...
    def setIp(self):
        ...
        self.drone_client.ws.setIp(data)

    def setPort(self):
        data = self.portEdit.toPlainText()
        self.drone_client.ws.setPort(data)

    def connect(self):
        """
        Connect to server
        :type: self.drone_client
        """
        if not self.drone_client.is_active():
            print("not connect")
            self.drone_client.ws.connect()
            return

        if self.drone_client.is_active():
            self.drone_client.ws.disconnect()
            return
    ...
    def arm(self):
        ...
        if not self.drone_client.is_active():
            ...
            return
        if self.drone_client.diagnostics.armed:
